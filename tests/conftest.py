...
from graphor_prd import GraphorPrd, AsyncGraphorPrd, DefaultAioHttpClient
from graphor_prd._utils import is_dict
...
logging.getLogger("graphor_prd").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[GraphorPrd]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with GraphorPrd(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncGraphorPrd]:
