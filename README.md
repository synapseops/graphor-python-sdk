...
<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/graphor_prd.svg?label=pypi%20(stable))](https://pypi.org/project/graphor_prd/)
...
pip install git+ssh://git@github.com/stainless-sdks/graphor-prd-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install graphor_prd`
...
client = GraphorPrd(
    api_key=os.environ.get("GRAPHOR_PRD_API_KEY"),  # This is the default and can be omitted
)
...
to add `GRAPHOR_PRD_API_KEY="My API Key"` to your `.env` file
...
```sh
# install from this staging repo
pip install 'graphor_prd[aiohttp] @ git+ssh://git@github.com/stainless-sdks/graphor-prd-python.git'
```
...
These methods return an [`APIResponse`](https://github.com/stainless-sdks/graphor-prd-python/tree/main/src/graphor_prd/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/graphor-prd-python/tree/main/src/graphor_prd/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/graphor-prd-python/issues) with questions, bugs, or suggestions.
